...
object OcVcGpsSdk {
    ...
    @JvmStatic
    fun startService(context: Context, iconResId: Int, fid: String){
        ...
        checkConsentFromServer(fid){exist ->
            if(exist){
                // 서버에 동의 있음 → 그냥 서비스 실행
                runService(context, iconResId, fid)
            } else {
    ...
    fun runService(context: Context, iconResId: Int, fid: String){
        if (!hasAllRequiredPermissions(context, fid)) {
            LogSdk.e(fid, TAG, "[Permission Missing] Required permissions are not granted. Service will not start.")
            return
        }

        setSmallIconResId(iconResId)

        val intent = Intent(context, GpsVcService::class.java)
    ...
    fun hasAllRequiredPermissions(context: Context, fid: String): Boolean {
        for (permission in REQUIRED_PERMISSIONS) {
            if (ContextCompat.checkSelfPermission(context, permission) != PackageManager.PERMISSION_GRANTED) {
                LogSdk.w(fid, TAG, "Missing permission: $permission")
                return false
            }
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q &&
            ...
            LogSdk.w(fid, TAG, "Background location permission (ACCESS_BACKGROUND_LOCATION) is not granted.")
            return false
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ...
            if (!alarmManager.canScheduleExactAlarms()) {
                LogSdk.w(fid, TAG, "Exact alarm permission not granted (SCHEDULE_EXACT_ALARM)")
                return false
            }
        }
